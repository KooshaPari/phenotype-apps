...
> Desktop-automation tier; now the home for the automation port traits · updated 2026-06-02
...
> **Status:** ACTIVE — rebuild foundation for the Phenotype eco-011 device-automation initiative (desktop tier). Companion repos: `KVirtualStage` (virtual-display/sandbox tier) and `kmobile` (mobile tier). The previous archive notice (and stray `KVirtualStage` heading) on this README was incorrect and has been replaced.
>
> **Naming:** This repository is `KDesktopVirt`. It is distinct from `KVirtualStage` / `kvirtualstage` / `kvs-demo`, which target the virtual-display / sandboxing layer. If you arrived here looking for the virtual-stage tier, see the `KVirtualStage` repo instead.
