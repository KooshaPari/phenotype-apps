...
impl SecurityManager {
    ...
    pub async fn unlock_vault(&mut self, password: String) -> Result<()> {
        ...
        let salt_path = self.vault_path.join("vault.salt");
        
        // Load or generate salt
        let salt = if salt_path.exists() {
            let salt_data = fs::read_to_string(&salt_path)
                .map_err(|e| anyhow!("Failed to read salt file: {}", e))?;
            SaltString::from_b64(&salt_data)
                .map_err(|e| anyhow!("Invalid salt format: {}", e))?
        } else {
            let new_salt = SaltString::generate(&mut OsRng);
            fs::write(&salt_path, new_salt.as_str())
                .map_err(|e| anyhow!("Failed to save salt: {}", e))?;
            new_salt
        };
    ...
    fn decrypt_credential(&self, credential: Credential) -> Result<Credential> {
        if let Some(master_key) = &self.master_key {
            ...
            // Decode and extract nonce from stored data
            ...
            if combined_data.len() < 12 {
                return Err(anyhow!("Invalid encrypted data: insufficient length"));
            }
