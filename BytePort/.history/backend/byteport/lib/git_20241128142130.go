...
func GetUserAccessToken(apiURL, jwtToken, code string) (string, error) {
	var secrets models.GitSecret 
	models.DB.First(&secrets)
	...
	var response struct {
		...
		refreshTokenExpires_in int `json:"refresh_token_expires_in"`
	}
	err = json.NewDecoder(resp.Body).Decode(&response)
	if err != nil {
		return "", fmt.Errorf("failed to parse User Access token response: %v", err)
	}
...
func refreshToken(accessToken string) (string, error) {
	...
	models.DB.First(&secrets)
	// GitHub API to generate installation token
	url := fmt.Sprintf("%s/login/oauth/access_token?client_id=%s&client_secret=%s&grant_type=refresh_token&refresh_token=%s", apiURL, secrets.ClientID, secrets.ClientSecret, code)
	req, err := http.NewRequest("POST", url, nil)
	if err != nil {
		return "", fmt.Errorf("failed to create request: %v", err)
	}
