...
func ValidateGit(apiURL, authMethod, authKey string) error {
	...
	var err error
	// Create request to fetch user information
	if authMethod == "token" {
		req, err = http.NewRequest("GET", apiURL+"/user", nil) // Adjust for GitHub, GitLab, etc.
		if err != nil {
			return fmt.Errorf("failed to create request: %v", err)
		}
		req.Header.Set("Authorization", "Bearer "+authKey)
	} else if authMethod == "ssh" {
		cmd := exec.Command("ssh", "-T", "git@"+strings.TrimPrefix(apiURL, "https://"))
		err := cmd.Run()
		if err != nil {
			return fmt.Errorf("failed to validate SSH connection: %v", err)
		}
		fmt.Println("SSH connection validated successfully.")
		return nil
	} else {
