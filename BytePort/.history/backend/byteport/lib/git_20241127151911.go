...
func ListRepositories(accessToken string) (*http.Response, error){
	...
	if err != nil {
		return resp,fmt.Errorf("failed to connect to GitHub API: %v", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return resp,fmt.Errorf("failed to fetch repositories: status code %d", resp.StatusCode)
	}
	
	fmt.Println("Successfully fetched repositories: status code %d", resp.StatusCode)
	fmt.
	return resp, nil
}
