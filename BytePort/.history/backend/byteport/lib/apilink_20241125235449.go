...
func ValidateGit(userID uint) error {
	...
	if result.Error != nil {
		return fmt.Errorf("failed to retrieve Git secrets for user %d: %v", userID, result.Error)
	}

	// Decrypt secrets
	clientID, err := DecryptSecret(gitSecrets.ClientID)
	if err != nil {
	...
	if gitSecrets.AccessToken == "" {
		return fmt.Errorf("GitHub is not linked for user %d. Access token is missing.", userID)
	}

	// Decrypt the stored user access token
	accessToken, err := DecryptSecret(gitSecrets.AccessToken)
	if err != nil {
