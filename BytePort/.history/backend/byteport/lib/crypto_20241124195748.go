...
func DecryptSecret(cipherText string, key string) (string, error) {
    ...
    const KeyFilePath = "./encryption_key"
...
func SaveKeyToFile(key string) error {
	err := ioutil.WriteFile(KeyFilePath, []byte(key), 0600) // Restrict access to the file
	if err != nil {
...
func GetOrCreateEncryptionKey() (string, error) {
	// Check if the key exists in the environment variable
	...
	if err == nil {
		// Add the key to the environment for the current runtime
	...
	// Save the key to the file for persistence
