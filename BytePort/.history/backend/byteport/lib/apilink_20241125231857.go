...
func LinkWithGithub(c *gin.Context, user models.User) {
	...
	https://github.com/login/oauth/authorize?client_id=YOUR_CLIENT_ID&state=STATE&scope=repo,user)
...
func ValidateGit(installationID string) error {
	...
	// Fetch Git secrets from the database
	...
	// Decrypt secrets
	appID, _ := DecryptSecret(gitSecrets.AppID)
	privateKey, _ := DecryptSecret(gitSecrets.PrivateKey)
