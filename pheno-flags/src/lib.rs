...
impl proptest::arbitrary::Arbitrary for FlagError {
    type Parameters = ();
    type Strategy = proptest::strategy::BoxedStrategy<Self>;

    fn arbitrary_with((): Self::Parameters) -> Self::Strategy {
        use proptest::strategy::Strategy;
        proptest::string::string_regex("[A-Z][A-Z0-9_]{2,32}")
            .expect("flag error regex")
            .prop_map(Self::InvalidValue)
            .boxed()
    }
}

impl proptest::arbitrary::Arbitrary for FlagSet {
    type Parameters = ();
    type Strategy = proptest::strategy::BoxedStrategy<Self>;
