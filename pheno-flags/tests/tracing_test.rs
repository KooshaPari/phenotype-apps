...
//! default code path (calls would be `#[cfg(feature = "tracing")]`
//! `tracing::info!` sites); this test exercises the dependency
...
fn emits_span_on_flag_lookup() {
    ...
    // Capture a span around a public API call.
    let span = tracing::info_span!("flag_lookup", key = "dark_mode");
    let _enter = span.enter();
    tracing::info!(value = flags.is_enabled("dark_mode"), "lookup");

    // The traced_test macro asserts the test function was actually
    // executed under a captured subscriber; if `tracing` is mis-
    // configured, the span above would not be recorded.
...
fn emits_span_on_from_env() {
    // Use a non-existent prefix to avoid reading the host env.
