...
| **ADR-036** **[ACCEPTED, EXECUTION DEFERRED 2026-06-20]** | **pheno-capacity substrate canonical** | L5-106, 2026-06-18 — see `docs/adr/2026-06-18/ADR-036-pheno-capacity.md`; **REVISION (2026-06-20):** the L5-115 extraction + L5-117 absorb work claimed DONE in v9 closure was **aspirational** — `KooshaPari/pheno-capacity` returns HTTP 404 (2026-06-20 orchestrator verify); `phenotype-gateway` is archived (`main` branch, archived 2026-06-20 08:52 UTC); no L5-117 branch was pushed to any remote. See `findings/2026-06-19-L5-117-pr-status.md` for the original drift log. **Action:** the L5-115/L5-117 work is deferred to a future v12+ batch; bucket stays at `HwLedger: from=PAUSED to=CONDITIONAL` (i.e. the extraction is not done); ADR-036 acceptance is **policy** (substrate canonical decision stands) but execution is **NOT COMPLETE**. |
