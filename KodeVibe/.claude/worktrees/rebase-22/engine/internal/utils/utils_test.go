...
func TestGenerateSecureToken(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Len(t, token16, 32) // 16 bytes = 32 hex chars
	...
	// Test that tokens contain only hex characters
	for _, c := range token {
...
func TestConstantTimeCompare(t *testing.T) {
	tests := []struct {
		...
		{[]byte(""), []byte(""), true},
		{[]byte("a"), []byte("b"), false},
		{[]byte("secret"), []byte("Secret"), false},
		{[]byte("long-token-value-123"), []byte("long-token-value-123"), true},
		{[]byte("short"), []byte("longer"), false},
	}

	for _, test := range tests {
		result := ConstantTimeCompare(test.a, test.b)
		assert.Equal(t, test.expected, result, "a: %s, b: %s", string(test.a), string(test.b))
	}

	// Verify constant-time property: should not leak length through timing
	assert.False(t, ConstantTimeCompare([]byte("a"), []byte("bb")))
	assert.False(t, ConstantTimeCompare([]byte("aa"), []byte("b")))
}
