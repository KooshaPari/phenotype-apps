...
// with the specified number of bytes (result is hex-encoded, so the output
// string length is 2*length). Uses crypto/rand for true randomness.
...
// to prevent timing side-channel attacks. Returns true if the slices are equal.
