...
// Test helpers for auth middleware

func setupAuthTestServer(secret string) *Server {
	config := &models.Configuration{
...
func TestServer_authMiddleware_MissingHeader(t *testing.T) {
	server := setupAuthTestServer("test-secret-key")
	...
	assert.Contains(t, response["message"], "Missing Authorization")
}

func TestServer_authMiddleware_InvalidFormat(t *testing.T) {
	server := setupAuthTestServer("test-secret-key")
	...
	require.NoError(t, err)
	assert.Equal(t, "unauthorized", response["error"])
	assert.Contains(t, response["message"], "Invalid Authorization")
}

func TestServer_authMiddleware_WrongToken(t *testing.T) {
	server := setupAuthTestServer("test-secret-key")
...
func TestServer_HealthRemainsPublicWhenAuthEnabled(t *testing.T) {
	server := setupAuthTestServer("test-secret-key")

	gin.SetMode(gin.TestMode)
	router := gin.New()
	// Simulate setupRoutes: no auth middleware on /health
	router.GET("/health", server.healthCheck)
	v1 := router.Group("/api/v1")
	v1.Use(server.authMiddleware())
	v1.GET("/config", server.getConfig)
...
func setupTestServerWithAuth(authEnabled bool, secret string) *Server {
	config := &models.Configuration{
		Server: models.ServerConfig{
			Auth: models.AuthConfig{
				Enabled: authEnabled,
...
func TestServer_authMiddleware_disabled(t *testing.T) {
	// Auth disabled should allow all requests
	server := setupTestServerWithAuth(false, "test-secret")
...
func TestServer_authMiddleware_enabled_valid(t *testing.T) {
	server := setupTestServerWithAuth(true, "my-super-secret-token")
...
func TestServer_authMiddleware_enabled_missingHeader(t *testing.T) {
	server := setupTestServerWithAuth(true, "secret")
	...
	err = json.Unmarshal(rr.Body.Bytes(), &response)
	require.NoError(t, err)
	assert.Contains(t, response["error"], "missing or malformed")
}

func TestServer_authMiddleware_enabled_wrongToken(t *testing.T) {
	server := setupTestServerWithAuth(true, "correct-secret")
	...
	err = json.Unmarshal(rr.Body.Bytes(), &response)
	require.NoError(t, err)
	assert.Contains(t, response["error"], "invalid authorization")
}

func TestServer_authMiddleware_enabled_noSecretConfigured(t *testing.T) {
	server := setupTestServerWithAuth(true, "") // Auth enabled but no secret
	...
	err = json.Unmarshal(rr.Body.Bytes(), &response)
	require.NoError(t, err)
	assert.Contains(t, response["error"], "misconfigured")
}

func TestServer_authMiddleware_enabled_badAuthScheme(t *testing.T) {
	server := setupTestServerWithAuth(true, "secret")
...
func TestServer_healthCheck_isPublic(t *testing.T) {
	// Health check should be accessible without auth even when auth is enabled
	server := setupTestServerWithAuth(true, "secret")

	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.GET("/health", server.healthCheck)
	// No auth middleware on health
