...
func (s *Server) setupRoutes(router *gin.Engine) {
	...
	// API v1 routes (protected by auth when enabled)
...
// authMiddleware enforces Bearer token authentication when auth is enabled
func (s *Server) authMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		authCfg := s.config.Server.Auth

		// If auth is disabled, let all requests through
		if !authCfg.Enabled {
			c.Next()
			return
		}

		// If no secret is configured, fail closed
		if authCfg.Secret == "" {
			s.logger.Error("Auth enabled but no secret configured")
		...
		if subtle.ConstantTimeCompare([]byte(token), []byte(authCfg.Secret)) != 1 {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{
