...
func (s *Server) Start(ctx context.Context, host string, port int, tlsEnabled bool, certFile, keyFile string) error {
	// Set Gin mode
	...
	go func() {
		if tlsEnabled {
			if certFile == "" || keyFile == "" {
				errCh <- fmt.Errorf("TLS certificate and key files are required for TLS mode")
				return
			}
	...
	// Block until context is cancelled (shutdown signal) or server fails
	select {
	case <-ctx.Done():
...
func (s *Server) setupRoutes(router *gin.Engine) {
	...
	// Protected API v1 routes (require auth when enabled)
...
// authMiddleware enforces API key authentication when server auth is enabled.
// It extracts the token from the Authorization header (Bearer scheme) and
// performs a constant-time comparison against the configured secret.
func (s *Server) authMiddleware() gin.HandlerFunc {
	cfg := s.config.Server.Auth
	return func(c *gin.Context) {
		authHeader := c.GetHeader("Authorization")
		if authHeader == "" {
			s.logger.Warn("Auth: missing Authorization header")
		...
		if len(authHeader) <= len(bearerPrefix) || authHeader[:len(bearerPrefix)] != bearerPrefix {
			s.logger.Warn("Auth: invalid Authorization header format")
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{
				"error":   "unauthorized",
		...
		if subtle.ConstantTimeCompare([]byte(token), []byte(cfg.Secret)) != 1 {
			s.logger.Warn("Auth: invalid token provided")
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{
				"error":   "unauthorized",
